...
#define CSIG0(x) (ROTRIGHT(x,2) ^ ROTRIGHT(x,13) ^ ROTRIGHT(x,22))
#define CSIG1(x) (ROTRIGHT(x,6) ^ ROTRIGHT(x,11) ^ ROTRIGHT(x,25))
...
void sha256_update(uint32_t state[8], const uint8_t data[], size_t len) {
    ...
    if (len > 0) {
        memcpy(buf, data, len);
        buf[len] = 0x80;  // Append the '1' bit
        memset(buf + len + 1, 0, sizeof(buf) - len - 1);
        if (len >= 56) {
            sha256_transform(state, buf);
            memset(buf, 0, 56);
        }
        *(uint32_t*)(buf + 60) = len * 8;  // Append length in bits
        sha256_transform(state, buf);
    }
}
